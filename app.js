  ...
  const rows = sorted.map((roster, idx) => {
    ...
    return `
      <tr class="${danger ? "relegation" : promoted ? "promotion" : ""}">
        ...
        <td>${pa}</td>
        <td>
          ${danger ? '<span class="relegation-tag">Relegation Risk</span>' : promoted ? '<span class="promotion-tag">Promotion Position</span>' : ""}
        </td>
      </tr>
    `;
  }).join("");
...
function renderTransactions(targetId, txs, users, rosters){
  ...
  if (!Array.isArray(txs) || !txs.length){
    el.innerHTML = "<p>No recent transactions found.</p>";
    return;
  }
...
async function loadLeagueView(leagueId, standingsTarget, txTarget, leagueType){
  ...
  // Look back through weeks for recent transactions
  let txs = [];
...
function renderMatchups(matchups, rosters, users, week){
  ...
  const html = grouped.map(group => {
    const a = group[0];
    const b = group[1];
    const aRoster = rosterMap.get(a.roster_id);
    const bRoster = b ? rosterMap.get(b.roster_id) : null;
    const aName = aRoster ? teamNameForRoster(aRoster, users) : `Roster ${a.roster_id}`;
    const bName = bRoster ? teamNameForRoster(bRoster, users) : (b ? `Roster ${b.roster_id}` : "Bye / TBD");
    const aPts = fmtNum(a.points);
    const bPts = fmtNum(b?.points);
    return `
      <div class="score-row">
        <div>
          <div><strong>${aName}</strong> vs <strong>${bName}</strong></div>
          <div class="score-meta">Most recent available results · Week ${week}</div>
        </div>
        <div class="score-points">${aPts} - ${bPts}</div>
      </div>
    `;
  }).join("");
...
async function loadHistory(){
  ...
  const championsChain = await fetchLeagueChain(CHAMPIONS_ID);
  const developmentChain = await fetchLeagueChain(DEVELOPMENT_ID);

  const bySeason = {};
  for (const league of championsChain) {
    bySeason[league.season] ||= {};
    bySeason[league.season].championsLeague = league;
  }
  for (const league of developmentChain) {
    bySeason[league.season] ||= {};
    bySeason[league.season].developmentLeague = league;
  }

  const seasons = Object.keys(bySeason)
    .filter(season => Number(season) >= 2023)
  ...
  const cards = seasons.map(season => {
    const c = bySeason[season].championsLeague;
    const d = bySeason[season].developmentLeague;
    return `
      <div class="history-season">
        <h3>${season}</h3>
        <p><strong>Champions League:</strong> ${c ? (c.name || "League found") : "Not found"}</p>
        <p><strong>Development League:</strong> ${d ? (d.name || "League found") : "Not found"}</p>
        <p>This season can be expanded later with champions, promotions, relegations, and final standings.</p>
      </div>
    `;
  }).join("");
...
async function loadTournament(){
  ...
  target.innerHTML = data.rounds.map(round => `
    <div class="bracket-round">
      <h3>${round.name}</h3>
      ${round.matches.map(match => `
        <div class="bracket-match">
          <div><strong>${match.team1}</strong> vs <strong>${match.team2}</strong></div>
          <div class="score-meta">${match.note || "Edit tournament.json to update this matchup."}</div>
        </div>
      `).join("")}
    </div>
  `).join("");
...
async function init(){
  ...
  const { week, matchups } = await getMostRecentWeekWithMatchups(CHAMPIONS_ID);
  if (week && matchups.length) {
    renderMatchups(matchups, leagueViews[0].rosters, leagueViews[0].users, week);
    status.textContent = `Showing most recent available Sleeper results · Week ${week}`;
  } else {
